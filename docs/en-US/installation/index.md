...
Once you've installed everything, will be copied automatically `config/default-config.yml` from `config/config.yml`. Any changes to `default-config.yml` will get overwritten when you update VVV, so always make changes to `config.yml`.
